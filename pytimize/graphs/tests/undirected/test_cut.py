...
class TestCut(TestCase):
    ...
    def test_cut_single(self) -> None:
        self.assertEqual(self.g.cut("a"), {("a", "b"), ("a", "w"), ("a", "s")}, "Should compute correct cut.")
        self.assertEqual(self.g.cut("b"), {("b", "c"), ("a", "b")}, "Should compute correct cut.")
    ...
    def test_cut_multiple(self) -> None:
        self.assertEqual(self.g.cut({"a", "b"}), {("a", "w"), ("a", "s"), ("b", "c")}, "Should compute correct cut.")
        self.assertEqual(self.g.cut({"a", "s"}), {("a", "w"), ("a", "b")}, "Should compute correct cut.")
        self.assertEqual(self.g.cut({"s", "w", "a"}), {("a", "b")}, "Should compute correct cut.")
    ...
    def test_cut_full(self) -> None:
        self.assertEqual(self.g.cut(set(self.g.vertices.keys())), set(), "Should return empty set.")
