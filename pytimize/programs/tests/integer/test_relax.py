...
class TestRelax(TestCase):
    def test_relax(self) -> None:
        ...
        self.assertTrue(
            np.allclose(lp.c, c), "Should have the same coefficient vector."
        )
        self.assertTrue(math.isclose(lp.z, z), "Should have the same constant.")
        self.assertEqual(
            lp.inequalities, ip.inequalities, "Should have the same inequalities."
        )
        self.assertEqual(lp.objective, ip.objective, "Should have the same objective.")
        self.assertEqual(
            lp.negative_variables,
            ip.negative_variables,
            "Should have the same negative variables.",
        )
        self.assertEqual(
            lp.free_variables, ip.free_variables, "Should have the same free variables."
        )
