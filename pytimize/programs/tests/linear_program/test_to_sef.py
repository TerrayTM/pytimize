...
class TestToSEF(TestCase):
    def test_to_sef(self):
        ...
        self.assertEqual(p.inequalities, ["="] * len(b), "Should compute correct inequalities in SEF.")
    ...
    def test_random_order_free_variables(self):
        ...
        self.assertTrue(np.allclose(p.b, np.array([10, 14, -2])), "Should compute correct constraint values in SEF.")
        self.assertTrue(np.allclose(p.c, np.array([-2, 2, 3, -3, -4, 1])), "Should compute correct coefficient vector in SEF.")
        self.assertTrue(math.isclose(p.z, 0), "Should compute correct constant in SEF.")
        self.assertEqual(p.inequalities, ["="] * len(b), "Should compute correct inequalities in SEF.")
        self.assertEqual(p.objective, "max", "Should be maximizing objective function in SEF.")
        self.assertEqual(p.free_variables, [], "Should have no free variables.")
    ...
    def test_random_order_negative_variables(self):
        ...
        p = LinearProgram(A, b, c, z, "max", free_variables=[2], negative_variables=[3, 1])
        ...
        self.assertTrue(np.allclose(p.b, np.array([10, 14, -2])), "Should compute correct constraint values in SEF.")
        self.assertTrue(np.allclose(p.c, np.array([-2, 3, -3, -4])), "Should compute correct coefficient vector in SEF.")
        self.assertTrue(math.isclose(p.z, 0), "Should compute correct constant in SEF.")
        self.assertEqual(p.inequalities, ["="] * len(b), "Should compute correct inequalities in SEF.")
        self.assertEqual(p.objective, "max", "Should be maximizing objective function in SEF.")
        self.assertEqual(p.free_variables, [], "Should have no free variables.")
