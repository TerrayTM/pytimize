...
class TestToCanonicalForm(TestCase):
    def setUp(self):
        self.A = np.array([[1, 1, 1, 0, 0], [2, 1, 0, 1, 0], [-1, 1, 0, 0, 1]])
    ...
    def test_compute_canonical_form(self):
        expected_A = np.array(
            [[1, 0, 0.5, 0, -0.5], [0, 1, 0.5, 0, 0.5], [0, 0, -1.5, 1, 0.5]]
        )
        ...
        self.assertTrue(
            np.allclose(result_one.A, expected_A),
            "Should compute correct coefficient matrix.",
        )
        self.assertTrue(
            np.allclose(result_one.b, expected_b),
            "Should compute correct constraint values.",
        )
        self.assertTrue(
            np.allclose(result_one.c, expected_c),
            "Should compute correct coefficient vector.",
        )
        self.assertTrue(
            math.isclose(result_one.z, expected_z), "Should compute correct constant."
        )

        expected_A = np.array(
            [[1, 0.5, 0, 0.5, 0], [0, 0.5, 1, -0.5, 0], [0, 1.5, 0, 0.5, 1]]
        )
        ...
        self.assertTrue(
            np.allclose(result_two.A, expected_A),
            "Should compute correct coefficient matrix.",
        )
        self.assertTrue(
            np.allclose(result_two.b, expected_b),
            "Should compute correct constraint values.",
        )
        self.assertTrue(
            np.allclose(result_two.c, expected_c),
            "Should compute correct coefficient vector.",
        )
        self.assertTrue(
            math.isclose(result_two.z, expected_z), "Should compute correct constant."
        )
    ...
    def test_non_sef(self):
        ...
        with self.assertRaises(
            ArithmeticError, msg="Should throw error for non-SEF linear programs."
        ):
