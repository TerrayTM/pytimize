...
class TestSimplex(TestCase):
    def test_simplex(self) -> None:
        ...
        self.assertTrue(
            np.allclose(solution, [4, 1, 2, 0, 0, 0, 0, 0]),
        ...
        self.assertTrue(
            np.allclose(solution, [5, 4, 0, 0, 0, 0]),
            "Should compute correct solution.",
        )
        self.assertEqual(basis, [1, 2, 5], "Should compute correct optimal basis.")
        self.assertTrue(
            np.allclose(certificate, [0.125, 0, 0.375]),
        ...
        self.assertTrue(
            np.allclose(solution, [3, 12, 0, 0, 3]), "Should compute correct solution."
        )
        self.assertEqual(basis, [1, 2, 5], "Should compute correct optimal basis.")
        self.assertTrue(
            np.allclose(certificate, [1.25, 0.25, 0]),
            "Should compute correct certificate.",
        )
        self.assertTrue(np.allclose(p.A, A), "Should not modify original program.")
    ...
    def test_simplex_unbounded(self) -> None:
        ...
        self.assertTrue(
            np.allclose(certificate[0], [36, 0, 14, 13, 0]),
            "Should be correct feasible solution.",
        )
        self.assertTrue(
            np.allclose(certificate[1], [0.5, 1, 0.5, 0, 0]),
            "Should be correct certifying vector.",
        )
    ...
    def test_simplex_optimal_form(self) -> None:
        A = np.array(
            [[1, 0, 0.75, -0.25, 0], [0, 1, -0.5, 0.5, 0], [0, 0, -1.75, 0.25, 1]]
        )
        b = np.array([3, 12, 3])
        ...
        self.assertEqual(basis, [1, 2, 5], "Should compute correct optimal basis.")
        self.assertTrue(
            np.allclose(certificate, [0, 0, 0]), "Should compute correct certificate."
        )
        self.assertTrue(np.allclose(p.A, A), "Should not modify original program.")
    ...
    def test_simplex_infeasible(self) -> None:
        ...
        with self.assertRaises(
            ArithmeticError, msg="Should throw exception if infeasible."
        ):
            p.simplex([4, 5, 6])
