...
class AbstractLinearProgram:
    ...
    def where(self, *constraints: VariableConstraint) -> "AbstractLinearProgram":
        for constraint in constraints:
            self._add_variable_constraint(
                constraint.positive_variables, self._positive_variables
            )
            self._add_variable_constraint(
                constraint.negative_variables, self._negative_variables
            )

        positive_set = set(self._positive_variables)
        negative_set = set(self._negative_variables)

        if not len(positive_set) == len(self._positive_variables) or not len(
            negative_set
        ) == len(self._negative_variables):
    ...
    def program(self) -> LinearProgram:
        if not self._program or self._recompile:
            ...
            free_variables = list(
                filter(lambda x: x not in union, range(1, self._c.shape[0] + 1))
            )
